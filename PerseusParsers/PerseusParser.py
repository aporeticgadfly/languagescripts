...
parser = argparse.ArgumentParser(prog='PerseusParser', description='scrapes Perseus Digital Library generated wordlist and outputs a list of all words to a local txt file')
...
with open(filename, 'w') as f:
    ...
    for row in rows:
        ...
        if len(columns) >= 9:
            ...
            print(f"Classical Word: {clas_word}, Weighted Frequency: {weighted_freq}, Definition: {definition}, Dictionary Links: {dict_links}, Principal Parts: {cleaned_text}")
            writer.writerow([clas_word, weighted_freq, definition, dict_links, cleaned_text])
            counter = counter + 1
