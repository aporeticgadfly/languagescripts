...
parser = argparse.ArgumentParser(prog='PerseusXMLParser', description='scrapes Perseus Digital Library generated XML and outputs a list of all words to a local txt file')
...
with open(filename, 'w') as f:
    writer = csv.writer(f)
    writer.writerow(["Classical Word", "Weighted Frequency", "Definition", "Dictionary Link", "Principal Parts"])
    ...
    for frequency_element in freq_elems:
        ...
        principal_parts_arr = parent_element.find_all('div div span strong').get_text().strip()
        print(principal_parts_arr)
        if principal_parts_arr:
            principal_parts = ", ".join(principal_parts_arr)

        print(f"Classical Word: {clas_word}, Weighted Frequency: {weighted_freq}, Definition: {definition}, Dictionary Links: {dict_links}, Principal Parts: {principal_parts}")
        writer.writerow([clas_word, weighted_freq, definition, dict_links, principal_parts])
        counter = counter + 1
