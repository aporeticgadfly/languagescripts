...
parser = argparse.ArgumentParser(
    prog="PDFGenderParser", description="gets plurality and gender of noun from PDF"
)
...
if __name__ == "__main__":
    with open(input_file, "r") as file:
        ...
        for line in csv_reader:
            if line[0][0].isupper():
                try:
                    wordparsed = wiktParser.fetch(line[0], "german")[0]["definitions"][
                        0
                    ]["text"][0]
                    if wordparsed:
                        ...
                        while iterator.isalpha():
                            iterator = wordparsed[
                                wordparsed.find("plural") + 7 + counter
                            ]
