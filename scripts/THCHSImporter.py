...
parser = argparse.ArgumentParser(
    prog="THCHSImporter.py",
    description="import THCHS corpus and outputs as anki readable csv file",
)
parser.add_argument("output", help="name of txt file to output to")
parser.add_argument(
    "input_file", help="name of folder where THCHS folder is downloaded"
)
        ...
        for file in files:
            if file.endswith(".mp3"):
                ...
                if corresponding_tr_file in files:
                    audio_path = file
                    with open(
                        os.path.join(root, corresponding_tr_file), "r", encoding="utf-8"
                    ) as tr_file:
